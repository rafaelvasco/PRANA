	...
	private void ApplyDeadZone(GamePadDeadZone dz)
	{
		switch (dz)
		{
			...
			case GamePadDeadZone.IndependentAxes:
				_left.X = Input.ExcludeAxisDeadZone(_left.X, Input.LEFT_DEAD_ZONE);
				_left.Y = Input.ExcludeAxisDeadZone(_left.Y, Input.LEFT_DEAD_ZONE);
				_right.X = Input.ExcludeAxisDeadZone(_right.X, Input.RIGHT_DEAD_ZONE);
				_right.Y = Input.ExcludeAxisDeadZone(_right.Y, Input.RIGHT_DEAD_ZONE);
				break;
			case GamePadDeadZone.Circular:
				_left = ExcludeCircularDeadZone(_left, Input.LEFT_DEAD_ZONE);
				_right = ExcludeCircularDeadZone(_right, Input.RIGHT_DEAD_ZONE);
				break;
		}
	}
...
{
    private static GamePadState[] _gpState;
    ...
    private static void InitGamepad()
    {
        ...
        if (ConnectedGamePads > 0)
        {
            ...
            {
                _gpState[i] = GetGamepadState((GamePadIndex)i);
            }
        }
    }

    private static void UpdateGpState()
    {
        if (ConnectedGamePads == 1)
        {
            _gpState[0] = GetGamepadState(GamePadIndex.One);
        }
        else
        {
            for (int i = 0; i < ConnectedGamePads; ++i)
    ...
    public static GamePadCapabilities GetGamepadInfo(GamePadIndex gamepadIndex)
    {
        return Platform.GetGamePadCapabilities((int)gamepadIndex);
    }

    public static GamePadState GetGamepadState(GamePadIndex gamepadIndex)
    {
    ...
    public static GamePadState GetGamepadState(GamePadIndex gamepadIndex, GamePadDeadZone deadZoneMode)
    {
        return Platform.GetGamePadState(
            (int)gamepadIndex,
            deadZoneMode
    ...
    public static bool SetGamepadVibration(GamePadIndex gamepadIndex, float leftMotor, float rightMotor)
    {
    ...
    public static string GetGamepadGuid(GamePadIndex gamepadIndex)
    {
        return Platform.GetGamePadGuid((int)gamepadIndex);
    }

    public static void SetGamepadLightBar(GamePadIndex gamepadIndex, Color color)
    {
