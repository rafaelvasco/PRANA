...
public struct KeyboardState
{
    ...
    private static readonly Key[] _pressedBuffer = new Key[Enum.GetValues(typeof(Key)).Length];
    ...
    public Span<Key> GetPressedKeys()
    {
        ...
        var index = 0;
        if (_keys0 != 0) index = AddKeysToArray(_keys0, 0 * 32, _pressedBuffer, index);
        if (_keys1 != 0) index = AddKeysToArray(_keys1, 1 * 32, _pressedBuffer, index);
        if (_keys2 != 0) index = AddKeysToArray(_keys2, 2 * 32, _pressedBuffer, index);
        if (_keys3 != 0) index = AddKeysToArray(_keys3, 3 * 32, _pressedBuffer, index);
        if (_keys4 != 0) index = AddKeysToArray(_keys4, 4 * 32, _pressedBuffer, index);
        if (_keys5 != 0) index = AddKeysToArray(_keys5, 5 * 32, _pressedBuffer, index);
        if (_keys6 != 0) index = AddKeysToArray(_keys6, 6 * 32, _pressedBuffer, index);
        if (_keys7 != 0) _ = AddKeysToArray(_keys7, 7 * 32, _pressedBuffer, index);

        return new Span<Key>(_pressedBuffer, 0, index);
    }
...
{
    public delegate void KeyInputEvent(Key key);
    ...
    public static bool EnableKeyboard { get; set; } = true;
    ...
    private static void InitKeyboard()
    {
        ...
        Platform.KeyDown = key => { OnKeyDown?.Invoke(key); };

        Platform.KeyUp = key => { OnKeyUp?.Invoke(key); };

        Platform.TextInput = c => { OnTextInput?.Invoke(c); };
    }

    private static void UpdateKbState()
    {
    }

    public static ref KeyboardState Key => ref _kbState;
